...
#DEFINE HMARGEM    030
#DEFINE VMARGEM    030
#DEFINE MAXITEM    019//022				// Máximo de produtos para a primeira página - Italac - Alterado por causa do redimensionamento dos quadros
...
#DEFINE MAXMENLIN  085//080				// Máximo de caracteres por linha de dados adicionais - Italac - Alterado por causa da alteração da fonte necessária para o Gestão do Leite
#DEFINE MAXMSG     018//013				// Máximo de dados adicionais por página - Italac - Quadro aumentado para comportar todas as mensagens do Leite sem gerar uma páxina extra
#DEFINE MAXVALORC  016//009				// Máximo de caracteres por linha de valores numéricos - Italac - Ajustado para tratar o aumento de casas decimais
#DEFINE MAXCODPRD  055//050				// Máximo de caracteres do codigo de produtos/servicos conforme o tamanho do quadro "Cod. prod" - Italac - Alterado para não quebrar o último dígito do código do produto
...
/*/
User Function RGLT076(cIdEnt, cVal1		, cVal2		, oDanfe,;   // User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
						oSetup, cFilePrint	, lIsLoja	, nTipo ,_cDir)
...
local lVerPerg	:= .T.
local lJob		:= .F.
local cProg		:= iif(existBlock("RGLT076A"),"U_RGLT076A","RGLT076A")  // iif(existBlock("DANFEProc"),"U_DANFEProc","DANFEProc")
...
If nTipo <> 1
	...
	If lJob .or. oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
		...
		// ----------------------------------------------
		oDanfe:cPathPDF := if ( lJob , _cDir , oSetup:aOptions[PD_VALUETYPE] ) // if ( lJob , SuperGetMV('MV_RELT',,"\SPOOL\") , oSetup:aOptions[PD_VALUETYPE] )
	elseIf oSetup:GetProperty(PD_PRINTTYPE) == IMP_SPOOL
		oDanfe:nDevice := IMP_SPOOL
		//oDanfe:SetParm( "-RFS") //Italac - Desabilita o parâmetro Real Font Sizes para não distorcer algumas customizações
		// ----------------------------------------------
	...
	Else
       If File(oDanfe:cSpoolLocal) 
          FErase(oDanfe:cSpoolLocal)
	   EndIf 
...
User Function RGLT076A(	oDanfe	, lEnd		, cIdEnt	, cVal1,; // DANFEProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,;
...
If !lImpDir .or. MV_PAR04 == 0 /* Caso impressão de DANFE seja realizada via AutoDistMail */
	...
	If !lPossuiF3 .And. MV_PAR04 <> 2
		...
		//Filtros Italac
		cWhere += " AND SF1.F1_FORNECE BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' AND SF1.F1_LOJA BETWEEN '"+MV_PAR13+"' AND '"+MV_PAR14+"' "
		//Se preencheu os setores, já fiz a validação de acesso no SX1
		If !Empty(MV_PAR15)
			cWhere += " AND SF1.F1_L_SETOR IN "+ FormatIn( AllTrim(MV_PAR15) , ';' )
		EndIf
		//Verifica se foi fornecido o filtro de linha
		If !Empty(MV_PAR16)
			cWhere += " AND SF1.F1_L_LINHA IN " + FormatIn(MV_PAR16,";")
		EndIf
...
ElseIf  lImpDir
	...
	if ( Val(MV_PAR02) - Val(MV_PAR01) ) > 0 .or. MV_PAR01 == MV_PAR02
		...
		ElseIf MV_PAR04==2
			...
			EndIF
			//Filtros Italac
			cWhere += " AND SF2.F2_CLIENTE BETWEEN '"+MV_PAR11+"' AND '"+MV_PAR12+"' AND SF2.F2_LOJA BETWEEN '"+MV_PAR13+"' AND '"+MV_PAR14+"' "
			cWhere += " AND SF2.F2_CARGA BETWEEN '"+MV_PAR09+"' AND '"+MV_PAR10+"' "
...
If nTipo <> 1
	...
	PRIVATE oFontTabu  := TFontEx():New(oDanfe,"Courier New"    ,07,07,.F.,.T.,.F.)// 13 Italac - Usada nas informações adicionais. Necessário para grade de Volume do Leite
...
Local aAreaSB5      := {}
Local aAreaDY3 		:= {}
Local _nPosH		:= 35 //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
Local nAuxH         := 0
Local nAuxH2        := 0
Local nSnBaseIcm	:= 0
Local nSnValIcm		:= 0
...
nAjustImp  := _nPosH //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
nAjustaRet := _nPosH //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
nAjustaEnt := _nPosH //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
nAjustaFat := _nPosH //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
nAjustaVt  := _nPosH //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
nAjustaPro := _nPosH //Italac - Necessário para reposicionar impressão por causa do código de barras customizado
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	...
	aadd(aMensagem,{cAux,.F.})//Italac - Criada quebra de linha própria, onde um é gerada uma nova linha a cada ;
	/*While !Empty(cAux)
		aadd(aMensagem, { SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)) , .F. } )
		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
	EndDo*/
...
//====================================================================================================
// Italac - Ajuste das quebras de linhas a cada ;
//====================================================================================================
ITQBRLIN( @aMensagem )
...
*/
//Italac - Customização do quadro para inclusão de códog de barras
oDanfe:Box(000,000,026,603)
oDanfe:Box(000,000,026,480)
oDanfe:Box(026,000,070,603)
oDanfe:Box(026,000,070,100)
oDanfe:Box(026,480,070,603)
oDanfe:Say(013, 002, "RECEBEMOS DE " + NoChar(oEmitente:_xNome:Text,lConverte) + " (CNPJ:" + TransForm(oEmitente:_CNPJ:TEXT,"@R! NN.NNN.NNN/NNNN-99") + ") OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO", oFont07:oFont)
oDanfe:Say(035, 002, "DATA DE RECEBIMENTO", oFont07N:oFont)
oDanfe:Say(062, 010, "________ / ________ / ________", oFont07N:oFont)
oDanfe:Say(035, 102, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont07N:oFont)
oDanfe:Say(068, 160, "CPF:", oFont07N:oFont)

// Código de barras para leitura de canhoto - Italac
_aSM0 		:= FWLoadSM0()	
_cfilial := _aSm0[ascan( _aSm0, { |x| x[18] = oEmitente:_CNPJ:TEXT})][2]  

oDanfe:code128c(055,505,_cfilial + StrZero(Val(oIdent:_NNf:Text),9) + oIdent:_Serie:Text , 28 )
oDanfe:code128c(068,105,_cfilial + StrZero(Val(oIdent:_NNf:Text),9) + oIdent:_Serie:Text , 14 )

oDanfe:Say(065, 525, _cfilial + " " +  StrZero(Val(oIdent:_NNf:Text),9) + " " +  oIdent:_Serie:Text, oFont08N:oFont) 
oDanfe:Say(006, 505, "N. Nfe                 Série", oFont08N:oFont)
oDanfe:Say(018, 505, StrZero(Val(oIdent:_NNf:Text),9) + "         " + oIdent:_Serie:Text, oFont08:oFont)

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Quadro 1 IDENTIFICACAO DO EMITENTE                                      ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

oDanfe:Box(_nPosH+042,000,_nPosH+137,250)
oDanfe:Say(_nPosH+052,096, "Identificação do emitente",oFont12N:oFont)
nLinCalc	:=	065 + _nPosH
...
oDanfe:Box(_nPosH+042,248,_nPosH+137,351)
if oDanfe:nDevice == 2
	oDanfe:Say(_nPosH+055,275, "DANFE",oFont12N:oFont)
else
	oDanfe:Say(_nPosH+055,275, "DANFE",oFont18N:oFont)
endif	 
oDanfe:Say(_nPosH+065,258, "DOCUMENTO AUXILIAR DA",oFont07:oFont)

if lNFCE
	oDanfe:Say(_nPosH+075,258, "NOTA FISCAL DE CONSUMIDOR",oFont07:oFont)
else
	oDanfe:Say(_nPosH+075,258, "NOTA FISCAL ELETRÔNICA",oFont07:oFont)
endif
oDanfe:Say(_nPosH+085,266, "0-ENTRADA",oFont08:oFont)
oDanfe:Say(_nPosH+095,266, "1-SAÍDA"  ,oFont08:oFont)
oDanfe:Box(_nPosH+078,315,_nPosH+095,325)
oDanfe:Say(_nPosH+089,318, oIdent:_TpNf:Text,oFont08N:oFont)
oDanfe:Say(_nPosH+110,255,"N. "+StrZero(Val(oIdent:_NNf:Text),9),oFont10N:oFont)
oDanfe:Say(_nPosH+120,255,"SÉRIE "+SubStr(oIdent:_Serie:Text,1,3),oFont10N:oFont)
oDanfe:Say(_nPosH+130,255,"FOLHA "+StrZero(nFolha,2)+"/"+StrZero(nFolhas,2),oFont10N:oFont)
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(_nPosH+045,003,cLogoD,090,090)
	Else
		oDanfe:SayBitmap(_nPosH+045,003,cLogo,090,090)
	EndIF
Endif
...
oDanfe:Box(_nPosH+042,350,_nPosH+088,603)
oDanfe:Box(_nPosH+075,350,_nPosH+110,603)
if oDanfe:nDevice == 2
	oDanfe:Say(_nPosH+095,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont09N:oFont)
else
	oDanfe:Say(_nPosH+095,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont12N:oFont)
endif

oDanfe:Box(_nPosH+105,350,_nPosH+137,603)

If nFolha == 1
	if oDanfe:nDevice == 2
		oDanfe:Say(_nPosH+085,355,"CHAVE DE ACESSO DA "+iif(lNFCE,"NFC-E","NF-E"),oFont09N:oFont)
	else
		oDanfe:Say(_nPosH+085,355,"CHAVE DE ACESSO DA "+iif(lNFCE,"NFC-E","NF-E"),oFont12N:oFont)
	endif
	
	nFontSize := 28
	oDanfe:Code128C(_nPosH+072,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
...
If Empty(cCodAutDPEC)
	If Empty(cChaveCont)
		if oDanfe:nDevice == 2
			oDanfe:Say(_nPosH+117,355,"Consulta de autenticidade no portal nacional da "+iif(lNFCE,"NFC-e","NF-e"),oFont09N:oFont)
			oDanfe:Say(_nPosH+127,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont09:oFont)
		else
			oDanfe:Say(_nPosH+117,355,"Consulta de autenticidade no portal nacional da "+iif(lNFCE,"NFC-e","NF-e"),oFont12:oFont)
			oDanfe:Say(_nPosH+127,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont12:oFont)
		endif
	Endif
Endif
...
If  !Empty(cCodAutDPEC)
	if oDanfe:nDevice == 2
		oDanfe:Say(_nPosH+117,355,"Consulta de autenticidade no portal nacional da "+iif(lNFCE,"NFC-e","NF-e"),oFont08:oFont)
		oDanfe:Say(_nPosH+127,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont08:oFont)
	else
		oDanfe:Say(_nPosH+117,355,"Consulta de autenticidade no portal nacional da "+iif(lNFCE,"NFC-e","NF-e"),oFont12:oFont)
		oDanfe:Say(_nPosH+127,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont12:oFont)
	endif
...
If !Empty(cChaveCont) .And. Empty(cCodAutDPEC) .And. !(Val(SubStr(oNF:_INFNFE:_IDE:_SERIE:TEXT,1,3)) >= 900)
	If nFolha == 1
		If !Empty(cChaveCont)
			nFontSize := 28
			oDanfe:Code128C(_nPosH+135,370,cChaveCont, nFontSize )
		EndIf
	Else
		If !Empty(cChaveCont)
			nFontSize := 28
			oDanfe:Code128C(_nPosH+112,370,cChaveCont, nFontSize )
		EndIf
	EndIf
EndIf

//Quadro 4
oDanfe:Box(_nPosH+139,000,_nPosH+162,603)
oDanfe:Box(_nPosH+139,000,_nPosH+162,350)
oDanfe:Say(_nPosH+148,002,"NATUREZA DA OPERAÇÃO",oFont08N:oFont)
oDanfe:Say(_nPosH+158,002,oIdent:_NATOP:TEXT,oFont08:oFont)

If !Empty(cCodAutDPEC)
	oDanfe:Say(_nPosH+148,352,"NÚMERO DE REGISTRO DPEC",oFont08N:oFont)
Endif
...
If Empty(cCodAutDPEC) .And. (((Val(SubStr(oNF:_INFNFE:_IDE:_SERIE:TEXT,1,3)) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1|6|7")
	oDanfe:Say(_nPosH+148,352,"PROTOCOLO DE AUTORIZAÇÃO DE USO",oFont08N:oFont)
Endif
If((oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"25")
	oDanfe:Say(_nPosH+148,352,"DADOS DA "+iif(lNFCE,"NFC-E","NF-E"),oFont08N:oFont)
Endif
...
oDanfe:Say(_nPosH+158,354,cDadosProt,oFont08:oFont)

nFolha++

//Quadro 5
oDanfe:Box(_nPosH+164,000,_nPosH+187,603)
oDanfe:Box(_nPosH+164,000,_nPosH+187,200)
oDanfe:Box(_nPosH+164,200,_nPosH+187,400)
oDanfe:Box(_nPosH+164,400,_nPosH+187,603)
oDanfe:Say(_nPosH+172,002,"INSCRIÇÃO ESTADUAL",oFont08N:oFont)
oDanfe:Say(_nPosH+180,002,IIf(Type("oEmitente:_IE:TEXT")<>"U",oEmitente:_IE:TEXT,""),oFont08:oFont)
oDanfe:Say(_nPosH+172,205,"INSC.ESTADUAL DO SUBST.TRIB.",oFont08N:oFont)
oDanfe:Say(_nPosH+180,205,IIf(Type("oEmitente:_IEST:TEXT")<>"U",oEmitente:_IEST:TEXT,""),oFont08:oFont)
oDanfe:Say(_nPosH+172,405,"CNPJ/CPF",oFont08N:oFont)
Do Case
...
oDanfe:Say(_nPosH+180,405,cAux,oFont08:oFont)
...
oDanfe:Say(_nPosH+195,002,"DESTINATARIO/REMETENTE",oFont08N:oFont)
oDanfe:Box(_nPosH+197,000,_nPosH+217,450)
oDanfe:Say(_nPosH+205,002, "NOME/RAZÃO SOCIAL",oFont08N:oFont)
oDanfe:Say(_nPosH+215,002,NoChar(oDestino:_XNome:TEXT,lConverte),oFont08:oFont)
oDanfe:Box(_nPosH+197,280,_nPosH+217,500)
oDanfe:Say(_nPosH+205,283,"CNPJ/CPF",oFont08N:oFont)
oDanfe:Say(_nPosH+215,283,cAux,oFont08:oFont)

oDanfe:Box(_nPosH+217,000,_nPosH+237,500)
oDanfe:Box(_nPosH+217,000,_nPosH+237,260)
oDanfe:Say(_nPosH+224,002,"ENDEREÇO",oFont08N:oFont)
oDanfe:Say(_nPosH+234,002,aDest[01],oFont08:oFont)
oDanfe:Box(_nPosH+217,230,_nPosH+237,380)
oDanfe:Say(_nPosH+224,232,"BAIRRO/DISTRITO",oFont08N:oFont)
oDanfe:Say(_nPosH+234,232,aDest[02],oFont08:oFont)
oDanfe:Box(_nPosH+217,380,_nPosH+237,500)
oDanfe:Say(_nPosH+224,382,"CEP",oFont08N:oFont)
oDanfe:Say(_nPosH+234,382,aDest[03],oFont08:oFont)

oDanfe:Box(_nPosH+236,000,_nPosH+257,500)
oDanfe:Box(_nPosH+236,000,_nPosH+257,180)
oDanfe:Say(_nPosH+245,002,"MUNICIPIO",oFont08N:oFont)
oDanfe:Say(_nPosH+255,002,aDest[05],oFont08:oFont)
oDanfe:Box(_nPosH+236,150,_nPosH+257,256)
oDanfe:Say(_nPosH+245,152,"FONE/FAX",oFont08N:oFont)
oDanfe:Say(_nPosH+255,152,aDest[06],oFont08:oFont)
oDanfe:Box(_nPosH+236,255,_nPosH+257,341)
oDanfe:Say(_nPosH+245,257,"UF",oFont08N:oFont)
oDanfe:Say(_nPosH+255,257,aDest[07],oFont08:oFont)
oDanfe:Box(_nPosH+236,340,_nPosH+257,500)
oDanfe:Say(_nPosH+245,342,"INSCRIÇÃO ESTADUAL",oFont08N:oFont)
oDanfe:Say(_nPosH+255,342,aDest[08],oFont08:oFont)

oDanfe:Box(_nPosH+197,502,_nPosH+217,603)
oDanfe:Say(_nPosH+205,504,"DATA DE EMISSÃO",oFont08N:oFont)
oDanfe:Say(_nPosH+215,504,Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT)),oFont08:oFont)
oDanfe:Box(_nPosH+217,502,_nPosH+237,603)
oDanfe:Say(_nPosH+224,504,"DATA ENTRADA/SAÍDA",oFont08N:oFont)
oDanfe:Say(_nPosH+233,504,Iif( Empty(aDest[4]),"",ConvDate(aDest[4]) ),oFont08:oFont)
oDanfe:Box(_nPosH+236,502,_nPosH+257,603)
oDanfe:Say(_nPosH+243,503,"HORA ENTRADA/SAÍDA",oFont08N:oFont)
oDanfe:Say(_nPosH+252,503,aHrEnt[01],oFont08:oFont)
...
//Calculo do ISSQN
/* Italac - retirado conforme permitido no manual para que as notas do leite caibam em 1 página
...
_nPosH:= 30
//Dados Adicionais
oDanfe:Say(719-_nPosH,000,"DADOS ADICIONAIS",oFont08N:oFont)
oDanfe:Box(721-_nPosH,000,865,351)
oDanfe:Say(729-_nPosH,002,"INFORMAÇÕES COMPLEMENTARES",oFont08N:oFont)
...
For nX := 1 To Min(nLenMensagens, MAXMSG)
	if aMensagem[nX][2]
		...
		oDanfe:Say( nLin, 002, aMensagem[nX][1], oFontTabu:oFont )
	endif
	nLin:= nLin+9//Italac - Diminuido o espaçamento entre as linhas para comportar todas as mensagens
Next nX
nMensagem := nX

oDanfe:Box(721-_nPosH,350,865,603)
oDanfe:Say(729-_nPosH,352,"RESERVADO AO FISCO",oFont08N:oFont)
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			IF MV_PAR05 <> 1 .And. nMensagem <= nLenMensagens
				...
				nLenMensagens:= Len(aMensagem)
				--nMensagem
				For nX := 1 To Min(nLenMensagens - nMensagem, MAXMSG-4)//Italac - retirada as linhas adicionais para a primeira página
	...
	Else
        ...
        //Italac - Retirado -2 pontos para não imprimir último díginto em cima da linha
...
If lMensagens
	...
	For nX := nMensagem To nForMensagens
		if aMensagem[nX][2]
			...
			oDanfe:Say( nLinha, 002, aMensagem[nX][1], oFontTabu:oFont )
		endif
		nMensagem++
		nLinha:= nLinha+ 10
	Next nX
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	*/		
	aTamCol := {50,;
				...
				24,;//22 Italac
				15,;
				iif(aCabec[4] == "CSOSN", 33, 40),; // Quant.//33,35 Italac
				iif(aCabec[4] == "CSOSN", 49, 60),; // V.Unitário//49/53 Italac
				40,;//38 Italac
				40,;//37 Italac
				39,;//32 Italac
				20,;//32 Italac
...
/*/
//-----------------------------------------------------------------------
Static Function IMPDNFLJ(oNFE, cCodAutSef, dDtReceb, cDtHrRecCab) // User Function IMPDNFLJ(oNFE, cCodAutSef, dDtReceb, cDtHrRecCab)
...
Data da Criacao---: 
===============================================================================================================================
Descrição---------: Realiza quebra das mensagens do quadro Informações Adicionais de acordo com regra da Italac
===============================================================================================================================
Parametros--------: aMensagem
===============================================================================================================================
Retorno-----------: Nenhum
===============================================================================================================================
*/
Static Function ITQBRLIN( aMensagem )

Local _aMsgAux	:= aClone(aMensagem)
Local _cMsgAux	:= ''
Local _cAux		:= ''
Local _nTamMsg	:= 0
Local _nI		:= 0
...
For _nI := 1 to Len(_aMsgAux)

	_cMsgAux := AllTrim( _aMsgAux[_nI][1] )
	
	While !Empty(_cMsgAux)
		
		_nTamMsg := At( ';' , _cMsgAux )
		
		If _nTamMsg > 0
			
			_cAux		:= SubStr( _cMsgAux , 1 , _nTamMsg - 1 )
			_cMsgAux	:= SubStr( _cMsgAux , _nTamMsg + 1 )
			
			While !Empty(_cAux)
				aAdd( aMensagem , {AllTrim( SubStr( _cAux , 1 , IIf( EspacoAt( _cAux , MAXMENLIN ) > 1 , EspacoAt( _cAux , MAXMENLIN ) - 1 , MAXMENLIN ) ) ),.F.})
				_cAux := SubStr( _cAux , IIf( EspacoAt( _cAux , MAXMENLIN ) > 1 , EspacoAt( _cAux , MAXMENLIN ) , MAXMENLIN ) + 1 )
			EndDo
			
		Else
		
			_cAux		:= AllTrim( _cMsgAux )
			_cMsgAux	:= ''
...
Data da Criacao---: 03/06/2024
===============================================================================================================================
Descrição---------: Verifica e retorna se há notas fiscais para geração do Pdf.
===============================================================================================================================
Parametros--------: Nenhum
===============================================================================================================================
Retorno-----------: _lRet = .T. = Existe notas para geração do PDF.
                            .F. = Não existe notas para a geração do PDF.
===============================================================================================================================
*/
User Function RGLT076H()
Local _lRet := .T.

Begin Sequence 
   //==============================================================================
   // Inicializa as variáveis MV_PARXX
   //==============================================================================
   Pergunte("NFSIGW",.F.)  
   ...
   If (cAliasSF1)->(Eof()) .Or. (cAliasSF1)->(Eof())
      _lRet := .F. 
   EndIf 
